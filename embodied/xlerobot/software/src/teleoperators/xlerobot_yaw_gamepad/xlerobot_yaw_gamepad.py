...
from ..xlerobot_gamepad.gamepad_utils import ALL_KEYMAP, get_gamepad_states, PS5Gamepad, print_decode_keymap
...
class XLeRobotYawGamepad(Teleoperator):
    ...
    def get_action(self) -> dict[str, float]:
        ...
        def _set_action_arm(prefix):
            ...
            dyaw = stepsize_ang * stepsize_factors_arm.get('yaw', 1)
            ...
            if states.get(f"{prefix}.wrist_yaw+", False) or states.get(f"{prefix}.wrist_yaw-", False):
                action[f"{prefix}_ee_delta.yaw"] = (dyaw if states.get(f"{prefix}.wrist_yaw+", False) else -dyaw)
